...
STATE="$(sketchybar --query $NAME | sed 's/\\n/\\\\n/g; s/\\\$/$/g' | jq -r '.geometry.background.drawing')"
...
case "$SENDER" in
  "mouse.clicked") toggle_menu
  ;;
  "front_app_switched") if [ $STATE = "on" ]; then update_menus; sleep 1; update_menus ; fi
  ;;
